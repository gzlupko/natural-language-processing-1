...
tidy_D1 %>%
  ...
  ylab("Mean Sentiment Score") + xlab("Week") + ggtitle("Mean Sentiment Score Per Week")
...
# below code using dtm.tfi threw an error re: non-zero entries; I ran LDA with dtm.tfi2 since it filtered out rows with non-zero entries 
...
#Convert corpus to a term document matrix - so each word can be analyzed individuallly
tdm.corpus.articles <- TermDocumentMatrix(articles_corpus)


# descriptive statistics for articles corpus 

findFreqTerms(tdm.corpus.articles, lowfreq = 500, highfreq = Inf) 
